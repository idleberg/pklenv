...
func TestLoadRejectsMalformedVariableName(t *testing.T) {
	...
	dir := t.TempDir()
	schema, err := filepath.Abs("../../schema/PklEnv.pkl")
	if err != nil {
		t.Fatal(err)
	}
...
func TestHintFiresOnTheRealDiagnostic(t *testing.T) {
	...
	if !strings.Contains(evalErr.Hint(), "cannot start with a digit") {
		t.Errorf("the name hint did not fire; the pattern in Hint has drifted from "+
			"EnvName in schema/PklEnv.pkl. Diagnostic was:\n%v", err)
	}
}
...
// TestVendoredTestdataSchemaIsCurrent guards the copy the fixtures amend.
//
// The fixtures use the vendored schema by relative name, the way a real config
// does, which means this directory holds a second copy of a file that lives at
// the repository root. A stale copy would quietly test the wrong contract, so
...
func TestVendoredTestdataSchemaIsCurrent(t *testing.T) {
	...
	if !strings.Contains(string(vendored), strings.TrimSpace(pklenv.Schema)) {
		t.Errorf("testdata/%s has drifted from schema/PklEnv.pkl; regenerate it", schema.Filename)
	}
}
