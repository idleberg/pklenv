...
func newRunCmd(stdio *IO, perms *perm.Set) *cobra.Command {
	var opts runOptions
...
untouched, which disables masking of the child's output.`),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return runRun(cmd, stdio, args, opts, perms)
		},
	}
...
func runRun(cmd *cobra.Command, stdio *IO, args []string, opts runOptions, perms *perm.Set) error {
	ctx := cmd.Context()

	warnStrictEnv(stdio, stdio.newLogger(opts.verbose))

	if err := ensureSchema(stdio, opts.file, opts.verbose); err != nil {
		return err
	}

	cfg, err := config.Load(ctx, opts.file, config.Options{Perm: *perms})
	if err != nil {
		return err
	}
