...
func NewRootCmd(io *IO) *cobra.Command {
	...
	root := &cobra.Command{
		...
		// --working-dir is applied here, before anything resolves a path.
		// Changing the process directory rather than threading a base path
		// through every call site is what makes one flag govern discovery, a
		// relative FILE, --outdir and the --root-dir default at once. It is also
		// why the child of `pklenv run` inherits the directory, which is the
		// intended reading of `pklenv -w ./services/api -- npm start`.
		PersistentPreRunE: func(cmd *cobra.Command, _ []string) error {
			if workingDir != "" {
				if err := os.Chdir(workingDir); err != nil {
					return withHint(
						fmt.Errorf("--%s: %w", perm.FlagWorkingDir, err),
						"the directory has to exist and be readable")
				}
			}
			wd, err := os.Getwd()
			if err != nil {
			...
			return perms.Resolve(wd)
		},
	}

	pf := root.PersistentFlags()
	pf.StringVarP(&workingDir, perm.FlagWorkingDir, "w", "",
		"run as if pklenv had been started in this directory")
	perm.Register(pf, &perms)

	root.AddCommand(newRunCmd(io, &perms), newEmitCmd(io, &perms))
