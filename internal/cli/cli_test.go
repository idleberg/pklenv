...
// writeConfig drops an env config into a temp dir.
//
// It amends the vendored schema by relative name, which is what a real config
// does and what the permission defaults require: the temp directory is the
// working directory, so an absolute path to the repository's own copy would sit
// outside the root boundary and be refused. pklenv writes the vendored file
// itself before evaluating, so these tests exercise that path on every run.
func writeConfig(t *testing.T, dir, name, body string) string {
	t.Helper()
	content := "amends \"" + schema.Filename + "\"\n\n" + body
