...
| `env.exfil.pkl`     | a URL built from a secret — refused by the network default             |
| `env.escape.pkl`    | a file read above the working directory — refused by the root boundary |
| `notenv.pkl`        | not a pklenv config at all; discovery skips it, naming it is an error  |

Because seven fixtures fail deliberately, a bare `pklenv emit` in this directory
...
| `pklenv emit env.unset.pkl`     | `Cannot find resource env:PKLENV_DEFINITELY_NOT_SET`     | unset _or_ not permitted by `--allow-env`; use `read?()` if absent |
...
The refusals, and the flag each one asks for. The first is the case the whole
model exists for: the secret must not appear in the output that reports the
block.

```sh
# denied by default; the hint names the origin, and the query string is gone
pklenv emit env.exfil.pkl
pklenv emit env.exfil.pkl --allow-net=example.com        # still refused: wrong host
pklenv emit env.exfil.pkl --allow-net=evil.example       # permitted

# file reads stop at the working directory
pklenv emit env.escape.pkl
pklenv emit env.escape.pkl --root-dir ../../..           # widened to the repo
pklenv emit env.escape.pkl --root-dir=''                 # boundary off

# environment grants are exact names, never prefixes
PKLENV_TEST_SECRET=x pklenv emit env.secretread.pkl --allow-env=PKLENV_TEST
PKLENV_TEST_SECRET=x pklenv emit env.secretread.pkl --allow-env=PKLENV_TEST_SECRET
```

`--verbose` is the one to check on a refusal: the URI stays scrubbed there too,
while Pkl's source excerpt still shows the line that caused it.

```sh
AWS_SECRET_ACCESS_KEY=AKIAnotreal pklenv emit env.exfil.pkl --verbose | grep AKIA   # no match
```

## Working directory

```sh
cd .. && pklenv emit -w testdata env.pkl --force   # discovery, output and root all move
pklenv run -w testdata -f env.pkl -- pwd           # the child lands there too
```
