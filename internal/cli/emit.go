...
func newEmitCmd(io *IO) *cobra.Command {
...
select from it in bulk with read*("env:PREFIX_*").`),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return runEmit(cmd, io, file, opts)
		},
	}
...
func runEmit(cmd *cobra.Command, io *IO, file string, opts emitOptions) error {
	ctx := cmd.Context()
	...
	for _, src := range sources {
		if err := emitOne(ctx, cmd, io, src, opts); err != nil {
...
func emitOne(ctx context.Context, cmd *cobra.Command, io *IO, src config.Source, opts emitOptions) error {
	cfg, err := config.Load(ctx, src.Path, config.Options{})
	if err != nil {
		return err
	}
